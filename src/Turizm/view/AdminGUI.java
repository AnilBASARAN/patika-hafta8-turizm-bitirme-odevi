...
import Turizm.business.AdminManager;
    ...
    public AdminGUI(User admin) {
        ...
        tbl_personel_popup.add("Güncelle").addActionListener(e -> {
            if (adminManager.updateUser(
                ...
                loadPersonelTable();
                Helper.s_Msg("Başarılı", "Kullanıcı bilgileri güncellendi");
            } else {
                Helper.s_Msg("Hata", "Kullanıcı bilgileri güncellenirken bir hata oluştu");
            }
        });
        tbl_personel_popup.add("Sil").addActionListener(e -> {
            if (adminManager.deleteUser(Integer.parseInt(tbl_personel.getValueAt(tbl_personel.getSelectedRow(), 0).toString()))) {
                loadPersonelTable();
                Helper.s_Msg("Başarılı", "Kullanıcı silindi");
            } else {
                Helper.s_Msg("Hata", "Kullanıcı silinirken bir hata oluştu");
            }
        });
        ...
        btn_addUser.addActionListener(e -> {
            if (Helper.isFieldEmpty(txt_tc) || Helper.isFieldEmpty(txt_userName) || Helper.isFieldEmpty(txt_password) || Helper.isFieldEmpty(txt_name) || Helper.isFieldEmpty(txt_surname)) {
                Helper.s_Msg("Uyarı", "Alanları eksiksiz doldurunuz");
            } else {
